"""DATASETS plugin — writes a calibration tensor batch to content-addressable cache.
...
from pet_quantize.calibration import vision_loader as _loader_mod


@DATASETS.register_module(name="vision_calibration_subset")
class VisionCalibrationSubset:
    """Produce a (num_samples, 3, 224, 224) tensor batch; cache under cache_dir."""
    ...
    def run(self, input_card: ModelCard, recipe: Any) -> ModelCard:
        """Load calibration images, stack into a batch tensor, and cache to disk."""
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        cache_path = self.cache_dir / f"{self._cache_key()}.pt"
        if not cache_path.exists():
            tensors = _loader_mod.load_calibration_images(self.source_uri, self.num_samples)
