"""DATASETS plugin — writes an audio calibration tensor batch to content-addressable cache.
...
from pet_quantize.calibration import audio_loader as _loader_mod


@DATASETS.register_module(name="audio_calibration_subset")
class AudioCalibrationSubset:
    """Produce a (num_samples, 1, 64, 100) tensor batch; cache under cache_dir."""
    ...
    def _cache_key(self) -> str:
        ...
        payload = f"audio|{self.source_uri}|{self.num_samples}".encode()
        return hashlib.sha256(payload).hexdigest()[:16]

    def run(self, input_card: ModelCard, recipe: Any) -> ModelCard:
        """Load calibration clips, stack into a batch tensor, and cache to disk."""
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        cache_path = self.cache_dir / f"{self._cache_key()}.pt"
        if not cache_path.exists():
            tensors = _loader_mod.load_calibration_clips(self.source_uri, self.num_samples)
